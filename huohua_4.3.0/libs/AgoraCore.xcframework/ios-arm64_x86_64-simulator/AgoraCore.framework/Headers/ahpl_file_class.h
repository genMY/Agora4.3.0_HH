...
public:
	...
	{
		return ahpl_co_file_read (fd, d_ret, d_buf);
	}

	int co_write (ahpl_data_t d_ret, ahpl_data_t d_buf)
	{
		return ahpl_co_file_write (fd, d_ret, d_buf);
	}

	int co_file_lseek (ahpl_data_t d_ret, off_t offset, int whence)
	{
		return ahpl_co_file_lseek (fd, d_ret, offset, whence);
	}
