...
@interface HHAgoraLiveVideoPlayer () <AVPlayerItemOutputPullDelegate, AgoraMediaPlayerDelegate>

@property (nonatomic, strong) dispatch_queue_t queue;

@property (nonatomic, strong) AgoraMediaPlayer *agoraMediaPlayer;
...
- (void)load {
    ...
    if (agoraLiveVideoPlayer) {
        ...
        _agoraMediaPlayer = [[AgoraMediaPlayer alloc] initWithDelegate:self voiceChatMode:enableVoiceChatMode];
        [_agoraMediaPlayer setPlayerOptionInt:@"che.player.audio.keep.audiosession" value:1];
    }
    ...
    if ([HHNavigationConfigManager isDefaultTurnOffWithName:@"AgoraMPKLogUpload"]) {
        ...
        [_agoraMediaPlayer setLogFile:[[[HHLiveLogManager share] getOrCreateLiveLogCacheDirectory:HHLiveLogTypeAgoraMediaPlayer] stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.log", fileName]]];
    }
...
- (void)destory {
    [_agoraMediaPlayer setView:nil];
    [_agoraMediaPlayer destroy];
    _agoraMediaPlayer = nil;
}
...
- (void)AgoraMediaPlayer:(AgoraMediaPlayer *_Nonnull)playerKit
    ...
    if ([[[HHLiveEngineManager share] getLiveEngine] isSupportLive]) {
        [HHLiveAgoraAudioFrameObserver pushAudioFrame:audioFrame];
    }
}
